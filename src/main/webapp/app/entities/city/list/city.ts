...
import { CityDeleteDialog } from '../delete/city-delete-dialog';
...
export class City implements OnInit {
  ...
  readonly isLoading = this.cityService.citiesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.cityService.citiesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.cities.set(this.fillComponentAttributesFromResponseBody([...this.cityService.cities()]));
    });
  ...
  trackId = (item: ICity): number => this.cityService.getCityIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && City.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  delete(city: ICity): void {
    const modalRef = this.modalService.open(CityDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && City.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }

  protected fillComponentAttributesFromResponseBody(data: ICity[]): ICity[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.cityService.citiesParams.set(queryObject);
  }
