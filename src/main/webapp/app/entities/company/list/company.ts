...
import { CompanyDeleteDialog } from '../delete/company-delete-dialog';
...
export class Company implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['legalName', 'tradeName', 'cnpj', 'stateRegistration', 'email', 'phone'];
  ...
  constructor() {
    effect(() => {
      const headers = this.companyService.companiesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.companies.set(this.fillComponentAttributesFromResponseBody([...this.companyService.companies()]));
    });
  ...
  trackId = (item: ICompany): number => this.companyService.getCompanyIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && Company.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && Company.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }

  protected fillComponentAttributesFromResponseBody(data: ICompany[]): ICompany[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.companyService.companiesParams.set(queryObject);
  }
