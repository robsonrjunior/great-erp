...
export class CountriesService {
  ...
  protected readonly resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/countries/_search');
}
...
export class CountryService extends CountriesService {
  ...
  search(req: SearchWithPagination): Observable<ICountry[]> {
    const options = createRequestOption(req);
    return this.http.get<ICountry[]>(this.resourceSearchUrl, { params: options }).pipe(catchError(() => scheduled([], asapScheduler)));
  }
