...
export class Supplier implements OnInit {
  ...
  constructor() {
    effect(() => {
      const headers = this.supplierService.suppliersResource.headers();
      if (headers) {
  ...
  search(query: string): void {
    ...
    if (query && predicate && Supplier.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && Supplier.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
