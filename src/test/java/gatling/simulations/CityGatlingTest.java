 ...
 */
public class CityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cities").get("/api/cities").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new city")
                        .post("/api/cities")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created city").get("${new_city_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created city").delete("${new_city_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the City entity").exec(scn);
