...
public class ProductGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all products").get("/api/products").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created product").get("${new_product_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created product").delete("${new_product_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
