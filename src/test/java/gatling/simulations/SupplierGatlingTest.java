...
public class SupplierGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all suppliers").get("/api/suppliers").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new supplier")
                        .post("/api/suppliers")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                    "\"legalName\": \"SAMPLE_TEXT\"" +
                                    ", \"tradeName\": \"SAMPLE_TEXT\"" +
                                    ", \"taxId\": \"SAMPLE_TEXT\"" +
                                    ", \"partyType\": \"PERSON\"" +
                                    ", \"email\": \"SAMPLE_TEXT\"" +
                                    ", \"phone\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_supplier_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created supplier").get("${new_supplier_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created supplier").delete("${new_supplier_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Supplier entity").exec(scn);
