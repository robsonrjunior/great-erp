...
public class TenantGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tenants").get("/api/tenants").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new tenant")
                        .post("/api/tenants")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tenant").get("${new_tenant_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created tenant").delete("${new_tenant_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Tenant entity").exec(scn);
