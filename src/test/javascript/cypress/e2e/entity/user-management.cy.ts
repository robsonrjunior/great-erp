...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Meire.Pereira24', email: 'Hugo62@yahoo.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Felix_Costa27');
      cy.get(`[data-cy="login"]`).should('have.value', 'Felix_Costa27');

      cy.get(`[data-cy="firstName"]`).type('Gabriel');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Gabriel');

      cy.get(`[data-cy="lastName"]`).type('Albuquerque');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Albuquerque');

      cy.get(`[data-cy="email"]`).type('Breno_Xavier90@hotmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Breno_Xavier90@hotmail.com');

      cy.get(`[data-cy="langKey"]`).select('es');
